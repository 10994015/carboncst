...
const oversea = ref({ ...DEFAULT_OVERSEA });
const isCreate = ref(false);
onMounted(() => {
  const overseaId = route.params.id;
  if (overseaId === "create") {
    randerLoading.value = true;
    oversea.value.id = overseaId;
    isCreate.value = true;
    return;
  }
  store
    .dispatch("isExistOversea", overseaId)
    .then((res) => {
      if (res.data) {
        store
          .dispatch("getOversea", overseaId)
          .then((res) => {
            oversea.value = res.data;
...
const previewImage = (ev) => {
  ...
  if (ev.target.files && ev.target.files[0]) {
    oversea.value.image = ev.target.files[0];
...
const uploadImages = (ev) => {
  ...
  if (ev.target.files && ev.target.files[0]) {
    Array.from(ev.target.files).forEach((file) => {
      oversea.value.images.push(file);
    });
...
const onSubmit = () => {
  ...
  if (isCreate.value) {
    store
      .dispatch("createOversea", oversea.value)
      .then((res) => {
    ...
    oversea.value["ori_images"] = images_url.value;
    store
      .dispatch("updateOversea", oversea.value)
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
...
<template>
  <div class="addOversea">
    <h1 v-if="isCreate">新增研討會</h1>
    <h1 v-else>編輯研討會</h1>
    <div class="card">
      ...
      <form v-if="randerLoading" action="" @submit.prevent="onSubmit()">
        <div class="form-group">
          ...
          <input type="text" v-model="oversea.title" />
        </div>
        <div class="form-group">
          <label for="">文章內容</label>
          <textarea id="editor1" name="editor1" v-model="oversea.content"></textarea>
        </div>
        <div class="form-group">
          <label for="">研討會連結</label>
          <input type="text" v-model="oversea.link" />
        </div>
        <div class="form-group">
        ...
        <div class="chkbox-group">
          <div class="form-group">
            ...
            <input type="checkbox" v-model="oversea.hidden" />
          </div>
        </div>
        <div class="form-group btn-group mt-10">
          ...
          <button
            class="pre"
            type="button"
            @click="router.push({ name: 'app.overseas' })"
          >
            回列表
          </button>
        </div>
