...
onMounted(() => {
    ...
    store.dispatch('isExistArticle', articleId).then(res=>{
        if(res.data){
            store.dispatch('getArticle', articleId).then(res=>{
                ...
                article.value.content = (article.value.content == 'null') ? "" :article.value.content;
...
const onSubmit = ()=>{
    loading.value = true;
    if(isCreate.value){
        store.dispatch('createArticle', article.value).then(res=>{
        ...
        store.dispatch('updateArticle', article.value).then(res=>{
            if(res.status === 200 || res.status === 201){
    ...
    <div class="card">
        ...
        <form v-if="randerLoading" action="" @submit.prevent="onSubmit()">
            ...
            <div class="form-group btn-group mt-10">
                ...
                <button class="pre" type="button" @click="router.push({name:'app.articles'})" >回列表</button>
            </div>
        </form>
