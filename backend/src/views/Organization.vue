...
import { ORGANIZATION_PER_PAGE } from "../constants";
import { useRouter, useRoute } from "vue-router";
const router = useRouter();
const perPage = ref(ORGANIZATION_PER_PAGE);
const search = ref("");
...
const getForPage = (ev, link) => {
  ...
  getOrganizations(link.url);
};

const getOrganizations = (url = null) => {
...
const selectedCheckItem = () => {
  if (checkedItems.value.length < organizations.value.total) {
...
const deleteCheckedItems = () => {
  if (confirm("確定刪除？")) {
    store.dispatch("deleteOrganizationItems", checkedItems.value).then((res) => {
      alert("刪除成功！");
      getOrganizations();
      checkedItems.value = [];
    });
    ...
    <div class="card">
      <div class="card-header">
        <div class="left">
          ...
          <div class="form-group">
            <select v-model="perPage" @change="getOrganizations()">
        ...
        <table class="table table-auto w-full animate-fade-in-down">
          <thead>
            <tr>
              ...
              <th
                @click="sortOrganizations('updated_at')"
          ...
          <tbody v-if="organizations.loading" class="loadingTable">
            <tr>
          ...
          <tbody v-else>
            <tr
              v-for="(organization, idx) of organizations.data"
              :key="organization.id"
            ...
            >
              ...
              <td class="w-[40px]">{{ organization.id }}</td>
              <td>
                <img v-if="organization.image_url" :src="organization.image_url" />
                <img v-else src="@/assets/news.jpg" />
              </td>
              <td>{{ organization.title }}</td>
              <td>{{ organization.updated_at }}</td>
              <td>
                <span v-if="organization.hidden">隱藏</span
              ...
              <td>
                ...
                <button class="delete ml-5" @click="deleteOrganization(organization)">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
      ...
      <div class="paging" v-if="organizations.total > organizations.limit">
        <div class="pageInfo">
          Showing from {{ organizations.from }} to {{ organizations.to }}
        </div>
        <div class="pageBtn">
          <nav>
            <a
              href="#"
              v-for="(link, i) of organizations.links"
              :key="i"
