...
const cstSeminar = ref({ ...DEFAULT_CSTSEMINAR });
const isCreate = ref(false);
onMounted(() => {
  const cstSeminarId = route.params.id;
  if (cstSeminarId === "create") {
    randerLoading.value = true;
    cstSeminar.value.id = cstSeminarId;
    isCreate.value = true;
    return;
  }
  store
    .dispatch("isExistCstSeminar", cstSeminarId)
    .then((res) => {
      if (res.data) {
        store
          .dispatch("getCstSeminar", cstSeminarId)
          .then((res) => {
...
const onSubmit = () => {
  ...
  if (isCreate.value) {
    store
      .dispatch("createCstSeminar", cstSeminar.value)
      .then((res) => {
    ...
    <div class="card">
      ...
      <form v-if="randerLoading" action="" @submit.prevent="onSubmit()">
        <div class="form-group">
          <label for="">研討會標題</label>
          <input type="text" v-model="cstSeminar.title" />
        </div>
        <div class="form-group">
          <label for="">研討會內容</label>
          <!-- <CKEditor :content="cstSeminar.content" @sendContent="getCkEditorContent" /> -->
          <textarea id="editor1" name="editor1" v-model="cstSeminar.content"></textarea>
        </div>
        <div class="form-group">
          <label for="">大會網站連結</label>
        ...
        <div class="form-group btn-group mt-10">
          ...
          <button
            class="pre"
            type="button"
            @click="router.push({ name: 'app.cst-seminar' })"
          >
            回列表
          </button>
        </div>
