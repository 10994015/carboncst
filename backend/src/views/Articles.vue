...
const getArticles = (url=null)=>{
    store.dispatch('getArticles', {url, sort_field:sortField.value, sort_direction: sortDirection.value, search: search.value, perPage:perPage.value});
}
    ...
    <div class="card">
        <div class="card-header">
            <div class="left">
                <div class="form-group">
                    ...
                    <input type="text" placeholder="搜尋..." v-model="search" @change="getArticles()" />
                </div>
                <div class="form-group">
            ...
            <div class="right">
                <div class="form-group">
                    <router-link class="btn" :to="{name:'app.add-article', params:{id:'create'}}">+ 新增文章</router-link>
                </div>
            </div>
        </div>
        <div class="table-responsive">
            <table class="table table-auto w-full animate-fade-in-down">
                <thead>
                    <tr>
                        <th class="w-[20px]"><input type="checkbox" v-model="isSelectAllChecked" @change="selectAllCheckItems()" /></th>
                        <th @click="sortArticles('id')" :class="['w-[40px]','cursor-pointer', {'active': sortField==='id'}]">
                        ...
                        <th @click="sortArticles('image')" :class="['cursor-pointer', {'active': sortField==='image'}]">
                            <div class="flex items-center ">
                                <div>圖片</div>
                        ...
                        <th @click="sortArticles('title')" :class="['cursor-pointer', {'active': sortField==='title'}]">
                            <div class="flex items-center ">
                                <div>標題</div>
                        ...
                        <th @click="sortArticles('updated_at')" :class="['cursor-pointer', {'active': sortField==='updated_at'}]">
                            <div class="flex items-center ">
                                <div>最後更新時間</div>
                ...
                <tbody v-else >
                    <tr v-for="(article, idx) of articles.data" :key="article.id" class="animate-fade-in-down" >
                        <td class="w-[20px]"><input type="checkbox" v-model="checkedItems" @change="selectedCheckItem()" ref="checkItem" :value="article.id" /></td>
                        <td class="w-[40px]">{{article.id}}</td>
                        <td><img :src="article.image_url" /></td>
                        <td>{{article.title}}</td>
                        <td>{{ article.updated_at }}</td>
                        <td><span v-if="article.hidden">隱藏</span><span v-else class="active">顯示</span></td>
                        <td>
                            <button class="edit ml-1" @click="router.push({name:'app.add-article', params:{id:article.id}})">
                    ...
                    <tr>
                        <td colspan="7">
                           <div class="flex items-center">
                                ...
                                <button :class="['bg-red-600', 'py-1', 'px-3', 'ml-4', 'text-white', 'rounded-sm', {'bg-gray-400':checkedItems.length <= 0}]" :disabled="checkedItems.length <= 0" @click="deleteCheckedItems()">一鍵刪除</button>
                           </div>
                        </td>
                    </tr>
                </tbody>
        ...
        <div class="paging" v-if="articles.total > articles.limit">
            ...
            <div class="pageBtn">
                <nav>
                    <a href="#" v-for="(link, i) of articles.links" :key="i" @click.prevent="getForPage($event, link)" :disabled="!link.url" :class="[{'active': link.active}, {'disabled':!link.url}]" v-html="link.label"></a>
                </nav>
            </div>
        </div>
    </div>
