...
const organization = ref({ ...DEFAULT_ORGANIZATION });
...
onMounted(() => {
  ...
  store
    .dispatch("isExistOrganization", organizationId)
    .then((res) => {
      if (res.data) {
        store
...
const previewImage = (ev) => {
  ...
  if (ev.target.files && ev.target.files[0]) {
    organization.value.image = ev.target.files[0];
...
const onSubmit = () => {
  ...
  if (isCreate.value) {
    store
      .dispatch("createOrganization", organization.value)
      .then((res) => {
    ...
    store
      .dispatch("updateOrganization", organization.value)
      .then((res) => {
        if (res.status === 200 || res.status === 201) {
    ...
    <div class="card">
      ...
      <form v-if="randerLoading" action="" @submit.prevent="onSubmit()">
        <div class="form-group">
          <label for="">標題</label>
          <input type="text" v-model="organization.title" />
        </div>
        <div class="form-group">
          <label for="">內容</label>
          <textarea v-model="organization.content"></textarea>
        </div>
        <div class="form-group">
          <label for="">圖片</label>
        ...
        <div class="chkbox-group">
          <div class="form-group">
            ...
            <input type="checkbox" v-model="organization.hidden" />
          </div>
        </div>
        ...
        <div class="form-group btn-group mt-10">
          ...
          <button
            class="pre"
            type="button"
            @click="router.push({ name: 'app.organization' })"
          >
            回列表
          </button>
        </div>
