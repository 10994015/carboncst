...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 電腦版用戶選單互動
    const userMenuLink = document.querySelector('.user-menu-link');
    if (userMenuLink) {
        const userDropdown = userMenuLink.querySelector('.user-dropdown');
    ...
    // 確保 isUser 狀態正確重置
    const navLinks = document.querySelectorAll('.navigation a[x-on\\:click]');
    ...
    // 添加載入動畫
    const authElements = document.querySelectorAll('.mobile-auth-section, .login-link, .user-menu-link');
    authElements.forEach(element => {
        element.classList.add('fade-in');
    });

    // 優化用戶體驗：預載入常用頁面
    const importantLinks = ['/profile', '/dashboard'];
